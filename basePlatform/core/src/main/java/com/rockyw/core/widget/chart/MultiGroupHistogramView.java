...
import com.rockyw.core.util.StringUtils;
...
public class MultiGroupHistogramView extends View {
    ...
    private int histogramValueDegree;
     ...
     * 上面的这个做法存在瑕疵，他只是对每一个阶梯中，每一种颜色做高低区分。那就存在一个阶梯内，数值大的比数值小的。还高的问题。
     * 所以，得用所有柱子里头，最高的那个值。做最高的对比
     */
    private float mChildrenMaxValue;
    ...
    private void init(AttributeSet attrs) {
        ...
        coordinateAxisWidth = typedArray.getDimensionPixelSize(R.styleable.MultiGroupHistogramView_coordinateAxisWidth, (int) DimensionUtil.dp(2));
        // 坐标轴线颜色
        ...
        groupNameTextSize = typedArray.getDimensionPixelSize(R.styleable.MultiGroupHistogramView_groupNameTextSize, (int) DimensionUtil.dp(15));
        groupInterval = typedArray.getDimensionPixelSize(R.styleable.MultiGroupHistogramView_groupInterval, (int) DimensionUtil.dp(30));
        histogramInterval = typedArray.getDimensionPixelSize(R.styleable.MultiGroupHistogramView_histogramInterval, (int) DimensionUtil.dp(10));
        // 直方图数值文本颜色
        ...
        histogramValueTextSize = typedArray.getDimensionPixelSize(R.styleable.MultiGroupHistogramView_histogramValueTextSize, (int) DimensionUtil.dp(12));
        histogramValueDecimalCount = typedArray.getInt(R.styleable.MultiGroupHistogramView_histogramValueDecimalCount, 0);
        histogramValueDegree = typedArray.getInt(R.styleable.MultiGroupHistogramView_histogramValueDegree, 0);
        histogramHistogramWidth = typedArray.getDimensionPixelSize(R.styleable.MultiGroupHistogramView_histogramHistogramWidth, (int) DimensionUtil.dp(20));
        chartPaddingTop = typedArray.getDimensionPixelSize(R.styleable.MultiGroupHistogramView_chartPaddingTop, (int) DimensionUtil.dp(10));
        histogramPaddingStart = typedArray.getDimensionPixelSize(R.styleable.MultiGroupHistogramView_histogramPaddingStart, (int) DimensionUtil.dp(15));
        histogramPaddingEnd = typedArray.getDimensionPixelSize(R.styleable.MultiGroupHistogramView_histogramPaddingEnd, (int) DimensionUtil.dp(15));
        distanceFormGroupNameToAxis = typedArray.getDimensionPixelSize(R.styleable.MultiGroupHistogramView_distanceFormGroupNameToAxis, (int) DimensionUtil.dp(15));
        distanceFromValueToHistogram = typedArray.getDimensionPixelSize(R.styleable.MultiGroupHistogramView_distanceFromValueToHistogram, (int) DimensionUtil.dp(10));
        typedArray.recycle();
...
//        }
        ...
        for (int i = 0, size = dataList.size(); i < size; i++) {
            MultiGroupHistogramGroupData groupData = dataList.get(i);
            ...
            if (childDataList != null && childDataList.size() > 0) {
                for (int j = 0; j < childDataList.size(); j++) {
                    ...
                    MultiGroupHistogramChildData childData = childDataList.get(j);
...
//                    }
                    if (mChildrenMaxValue < childData.getValue()){
                        mChildrenMaxValue = childData.getValue();
                    }
                }
            ...
            for (int i = 0, size = dataList.size(); i < size; i++) {
                ...
                if (childDataList != null && childDataList.size() > 0) {
                    ...
                    for (int j = 0; j < childDataList.size(); j++) {
                        ...
                        if (childData.getValue() <= 0 || mChildrenMaxValue <= 0) {
                            childHistogramHeight = 0;
                        } else {
                            childHistogramHeight = (int) (childData.getValue() / mChildrenMaxValue * maxHistogramHeight);
                        }
                        ...
                        int[] histogramShaderColor = histogramShaderColorArray.get(j);
                        ...
                        String childHistogramHeightValue = StringUtils.getNumericScaleByFloor(String.valueOf(childData.getValue()), histogramValueDecimalCount) + childData.getSuffix();
                        ...
                        if (histogramValueDegree == 0) {
                            canvas.drawText(childHistogramHeightValue, valueTextX, valueTextY, histogramValuePaint);
                        } else {
                            //#这里客制化角度
                            canvas.save();
                            canvas.rotate(histogramValueDegree, valueTextX + histogramValuePaint.measureText(childHistogramHeightValue), valueTextY);
                            float xOffsetCoefficient = (float) Math.sin(Math.PI * histogramValueDegree / 180);
                            float yOffsetCoefficient = (float) Math.cos(Math.PI * histogramValueDegree / 180);
                            float adjustValueTextX = valueTextX - histogramValuePaint.measureText(childHistogramHeightValue) * xOffsetCoefficient;
                            float adjustValueTextY = valueTextY - histogramValuePaint.measureText(childHistogramHeightValue) * yOffsetCoefficient;
                            canvas.drawText(childHistogramHeightValue, adjustValueTextX, adjustValueTextY, histogramValuePaint);
                            canvas.restore();
                        }

                        int deltaX = j < childDataList.size() - 1 ? histogramHistogramWidth + histogramInterval : histogramHistogramWidth;
                        groupWidth += deltaX;
                        xAxisOffset += j == childDataList.size() - 1 ? deltaX + groupInterval : deltaX;
                    }
