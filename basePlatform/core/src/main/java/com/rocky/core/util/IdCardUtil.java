...
public class IdCardUtil {
    ...
    public static boolean verify(String idcard) {
        ...
        //如果是15位的就转成18位的身份证
        String eifhteenCardId = "";
        if (idcard.length() == 15) {
            eifhteenCardId = upToEighteen(idcard);
        } else {
            eifhteenCardId = idcard;
        }
        //验证身份证的地区码
        if (!verifyAreaCode(eifhteenCardId)) {
            return false;
        }
        //判断月份和日期
        if (!verifyBirthdayCode(eifhteenCardId)) {
            return false;
        }
        //验证18位校验码,校验码采用ISO 7064：1983，MOD 11-2 校验码系统
        return verifyMOD(eifhteenCardId);
    }
